...
If your app uses Tailwind, Roster must come **first**:
...
Roster's styles live in a `roster` cascade layer, and CSS orders layers by
*first registration*. Importing Roster first puts its layer beneath your app's
utilities, so your styles always win. Import it second and the reverse happens:
Roster's `.block` starts beating your `dark:hidden`, and variants fail silently
— the rule is right there in the stylesheet, and simply loses.
